...
function CompanyDetailPage() {
    ...
    try {
      if (!isValidUuid) {
        ...
        try {
          ...
          const url = `${API_BASE_URL}/companies/${companyId}/jobs${queryString ? `?${queryString}` : ''}`
          ...
          try {
            const altResponse = await fetch(`${API_BASE_URL}/jobs/company/${companyId}`)
            if (altResponse.ok) {
  ...
  const fetchCompanyPhotos = useCallback(async () => {
    try {
      ...
      const resp = await fetch(`${API_BASE_URL}/companies/${companyId}/photos`)
      if (resp.ok) {
