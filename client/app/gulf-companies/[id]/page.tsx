...
import { apiService, Job, Company, API_BASE_URL, BASE_URL } from '@/lib/api'
...
function GulfCompanyDetailPage() {
  ...
  const fetchCompanyJobs = useCallback(async (filterParams: any = {}) => {
    setLoadingJobs(true)
    setJobsError("")
    try {
      ...
      if (!isValidUuid) {
        ...
        try {
          ...
          try {
            ...
            const url = `${API_BASE_URL}/companies/${companyId}/jobs?${queryParams.toString()}`
  ...
  const fetchCompanyPhotos = useCallback(async () => {
    try {
      ...
      const resp = await fetch(`${API_BASE_URL}/gulf-companies/${companyId}/photos`)
      if (resp.ok) {
  ...
  const handlePhotoDelete = useCallback(async (photoId: string) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/gulf-companies/photos/${photoId}`, {
        method: 'DELETE',
      ...
      <div className="min-h-screen bg-animated dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 relative">
        ...
        <div className="relative w-full pt-16">
          ...
          <div className="relative w-full h-[35vh] min-h-[400px] overflow-hidden">
            {company.banner ? (
              <img
                src={company.banner.startsWith('http') ? company.banner : `${BASE_URL}${company.banner}`}
                  ...
                  <div className="flex-1 text-white">
                    <div className="flex items-center space-x-4 mb-4">
                      <Avatar className="w-20 h-20 ring-4 ring-white/30 shadow-xl">
                        <AvatarImage
                          src={company.logo ? (company.logo.startsWith('http') ? company.logo : `${BASE_URL}${company.logo}`) : "/placeholder.svg"}
                                    ...
                                    <div className="flex items-start justify-between mb-4">
                                      ...
                                      <div className="flex flex-col space-y-2">
                                        <Button
                                          ...
                                          className={`h-10 px-6 ${hasAppliedToJob(job.id)
                                    ...
                                    <div className="flex items-center justify-between pt-4 border-t border-slate-200 dark:border-slate-700">
                                      ...
                                      <div className="flex items-center space-x-2">
                                        ...
                                        {user && user.userType === 'jobseeker' ? (
                                          <Button
                                            size="sm"
                                            className={`${hasAppliedToJob(job.id)
                                              ? 'bg-green-600 hover:bg-green-700 cursor-default'
                                              : job.status === 'expired'
                                                ? 'bg-slate-300 cursor-not-allowed'
