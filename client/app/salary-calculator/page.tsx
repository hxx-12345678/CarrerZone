...
const SalaryCalculator = () => {
  ...
  const fetchAvailableStates = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/salary/states`);
  ...
  const fetchAvailableRegimes = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/salary/regimes`);
      const data = await response.json();
      if (data.success) {
                ...
                <div className="grid grid-cols-1 gap-3">
                  {availableRegimes.map((regime) => {
                    ...
                    return (
                      <div
                        key={regime}
                        className={`p-3 rounded-lg border-2 cursor-pointer transition-all ${selectedRegimes.includes(regime)
                      ...
                      >
                        <div className="flex items-center justify-between">
                          ...
                          <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${selectedRegimes.includes(regime)
                  ...
                  <CardContent className="p-6">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      {Object.entries(result.regimes).map(([regimeKey, regime]) => {
                        ...
                        return (
                          <div key={regimeKey} className={`relative p-4 rounded-lg border-2 ${isBestOption
                            ...
                            <div className="text-center">
                              <h3 className={`text-lg font-semibold mb-2 ${isBestOption ? 'text-green-800' : 'text-gray-800'
                                }`}>
                                {regime.regime}
                              </h3>
                    ...
                    <div className="space-y-4">
                      {Object.entries(result.regimes).map(([regimeKey, regime]) => {
                        ...
                        return (
                          <div
                            key={regimeKey}
                            className={`p-6 rounded-xl border-2 transition-all ${isBest
                          ...
                          return (
                            <div key={regimeKey} className="space-y-2">
                              ...
                              <div className="w-full bg-gray-200 rounded-full h-3">
                                <div
                                  className={`h-3 rounded-full transition-all duration-1000 ${isLowest ? 'bg-gradient-to-r from-green-400 to-green-600' : 'bg-gradient-to-r from-red-400 to-red-600'
                                    }`}
